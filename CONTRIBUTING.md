...
Thank you for contributing to Robin! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [EnergyIR Discord](https://discord.gg/NousResearch). Users can install it with `hermes skills install`.
...
│
├── robin/               # CLI command implementations
...
├── tests/                    # Test suite
├── website/                  # Documentation site (robin.energyir.com)
│
...
Add to `_BUILTIN_SKINS` dict in `robin/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `robin/skin_engine.py` for the full schema and existing skins as examples.
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `robin/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `robin/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `robin/gateway_windows.py::
