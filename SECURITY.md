# Robin Security Policy

This document describes Robin's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/dmjdxb/Robin/security/advisories/new)
or **security@energyir.com**. Do not open public issues for
security vulnerabilities. **Robin does not operate a bug
bounty program.**
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Robin access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Robin's documentation
...
Robin supports two OS-level isolation postures. They address
...
Robin supports this in two ways:

- **Robin's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Robin's in-process heuristics
...
Robin filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Robin
itself. Bugs in Robin's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Robin does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Robin's own documentation, or reasonable
  operator expectations would predict — including cases where
  Robin has documented a stance about how its output should
...
- Robin includes supply-chain guards for MCP server
