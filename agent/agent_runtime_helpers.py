...
def try_recover_primary_transport(
    ...
    Skipped for proxy/aggregator providers (OpenRouter, EnergyIR) which
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    # Together AI proxies to OpenRouter behind the scenes — identical
    ...
    if (is_openrouter or is_nous_portal) and is_claude:
        return True, False
    # Together AI Qwen (e.g. qwen3.6-plus) takes the same envelope-layout
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    if hasattr(agent, "context_compressor") and agent.context_compressor:
        ...
        try:
            from robin.config import load_config, get_compatible_custom_providers
            _sm_cfg = load_config()
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    if not pre_tool_block_checked:
        try:
            from robin.plugins import get_pre_tool_call_block_message
