...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "RobinAgent",
}
...
# in lockstep with robin.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``robin.__version__`` is
...
_NOUS_DEFAULT_BASE_URL = "https://inference-api.energyir.com/v1"
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``robin.runtime_provider._detect_api_mode_for_url`` so the
...
def _nous_api_key(provider: dict) -> str:
    ...
    from robin.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    """
    try:
        from robin.auth import resolve_nous_runtime_credentials
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    Falls back to ``robin.auth``'s singleton runtime resolver for older
    ...
    try:
        from robin.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    ...
    try:
        from robin.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from robin.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from robin.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from robin.models import copilot_default_headers
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    model = _NOUS_MODEL
    try:
        from robin.models import get_nous_recommended_aux_model
...
def _try_azure_foundry(
    *,
    ...
    :func:`robin.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from robin.runtime_provider import _resolve_azure_foundry_runtime
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    ...
    try:
        from robin.nous_account import get_nous_portal_account_info
...
def _recoverable_pool_provider(
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "inference-api.energyir.com"):
        return "nous"
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from robin.copilot_auth import copilot_request_headers
...
def _normalize_resolved_model(model_name: Optional[str], provider: str) -> Optional[str]:
    ...
    if not model_name:
        return model_name
    try:
        from robin.model_normalize import normalize_model_for_provider
...
def resolve_provider_client(
    ...
    try:
        from robin.runtime_provider import _get_named_custom_provider
    ...
    try:
        ...
        logger.debug("robin.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from robin.models import _should_use_copilot_responses_api
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`robin.plugins.PluginContext.register_auxiliary_task`) the
