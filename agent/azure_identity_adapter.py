...
* No persisted JWT. ``azure-identity`` caches in-process and (where
  available) in the OS keychain or ``~/.IdentityService``. Robin does
...
class EntraIdentityConfig:
    ...
    Captures the Robin-managed Entra knobs we need outside Azure SDK
    ...
    ``robin/runtime_provider.py:1310-1377`` for the analogous
...
def _build_default_credential(config: EntraIdentityConfig) -> Any:
    ...
    Only Robin-selected knobs are passed as kwargs. Everything else
...
def build_credential(config: EntraIdentityConfig) -> Any:
    ...
    Robin processes use exactly one Entra config at a time (the
...
def materialize_bearer_for_http(value: Any) -> str:
    ...
    header outside the OpenAI SDK (e.g. ``robin/azure_detect.py``).
