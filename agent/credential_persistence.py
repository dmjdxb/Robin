...
``auth.json``.  They intentionally have no dependency on ``robin.auth`` so
...
# Sources Robin owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Robin-owned OAuth/device-code state)
