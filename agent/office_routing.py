...
def office_directive() -> str:
    ...
    return (
        ...
        "parallel workers.\n"
        "4. For illustrations, generate the image files with matplotlib/PIL via execute_code "
        "(reliable and already available), or the image_generate tool; reference them as image "
        "blocks — that is allowed.\n"
        "5. Build by calling the build_document (docx) or build_presentation (pptx) tool "
        "DIRECTLY with a content spec — these are in your tool list. The designed templates own "
        "all layout. Do NOT run a terminal python script and do NOT import docx/pptx yourself.\n"
