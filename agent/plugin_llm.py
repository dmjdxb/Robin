...
Plugins built on Robin often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Robin
...
:class:`~robin.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Robin' main config
...
handles every provider, fallback chain, and per-task override Robin
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`robin.plugins.PluginContext`
