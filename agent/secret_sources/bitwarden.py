...
Robin pulls API keys from Bitwarden Secrets Manager at process startup
...
* The ``bws`` binary is auto-installed into ``<hermes_home>/bin/bws`` on
  first use.  Robin pins one version (``_BWS_VERSION``) and downloads
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  hammer the API.
* Failures NEVER block Robin startup.  Missing binary, no network,
...
def _hermes_bin_dir() -> Path:
    """Where Robin stores its managed binaries.  Profile-aware."""
