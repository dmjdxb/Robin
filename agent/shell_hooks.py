...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`robin.plugins.invoke_hook` and its aggregators.  Python
...
* Registration is idempotent — safe to invoke from both the CLI entry
  point (``robin/main.py``) and the gateway entry point
  (``gateway/run.py``).
...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    {"action":   "block", "message": "Forbidden command"}   # Robin-canonical
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``robin.config.load_config``
...
def run_once(
    ...
    ``kwargs`` is the same dict that :func:`robin.plugins.invoke_hook`
    ...
    holding the canonical Robin-wire-shape response."""
