...
app.setAboutPanelOptions({
  ...
  copyright: 'Copyright © 2026 EnergyIR'
})
...
function findGitBash() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'robin', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'robin', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'robin', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'robin', 'git', 'bin', 'git.exe'))
  }
...
// `hermes update`). Instead we spawn the staged Robin-Setup binary with
...
async function applyUpdatesPosixInApp() {
  ...
  // Put the Robin-managed Node and the venv on PATH so `hermes desktop`'s
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-Robin-Session-Token`` header — used by the auth-mode probe before
...
//   * Together AI now issues a 24h ROTATING, reuse-detected refresh token
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with EnergyIR" instead of a generic
