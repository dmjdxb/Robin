 ...
 *
 * Stamps the Robin icon + identity onto the packed Windows Robin.exe via
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Robin')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Robin'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Robin.exe keeps the stock Electron icon`)
  }
}
