...
import type { RobinReadDirResult } from '@/global'

import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<RobinReadDirResult>>()
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): RobinReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: RobinReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<RobinReadDirResult>(resolve => {
