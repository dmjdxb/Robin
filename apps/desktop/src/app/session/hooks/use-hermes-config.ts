...
import { getRobinConfig, getRobinConfigDefaults } from '@/hermes'
...
interface RobinConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useRobinConfig({ activeSessionIdRef, refreshProjectBranch }: RobinConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshRobinConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getRobinConfig(), getRobinConfigDefaults().catch(() => ({}))])
  ...
  return { refreshRobinConfig, sttEnabled, voiceMaxRecordingSeconds }
}
