...
declare global {
  interface Window {
    hermesDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<RobinConnection>
      ...
      api: <T>(request: RobinApiRequest) => Promise<T>
      notify: (payload: RobinNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<RobinReadFileTextResult>
      selectPaths: (options?: RobinSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<RobinPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<RobinPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: RobinTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<RobinReadDirResult>
      resolveWorkspace: (rememberedCwd: string) => Promise<string>
      gitRoot?: (path: string) => Promise<string | null>
      terminal: {
        dispose: (id: string) => Promise<boolean>
        onData: (id: string, callback: (payload: string) => void) => () => void
        onExit: (id: string, callback: (payload: RobinTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<RobinTerminalSession>
        write: (id: string, data: string) => Promise<boolean>
      }
      onClosePreviewRequested?: (callback: () => void) => () => void
      onOpenUpdatesRequested?: (callback: () => void) => () => void
      onWindowStateChanged?: (callback: (payload: RobinWindowState) => void) => () => void
      onPreviewFileChanged: (callback: (payload: RobinPreviewFileChanged) => void) => () => void
...
export interface RobinTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface RobinTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `robin update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  hermesRoot?: string
}
...
export interface RobinConnection {
  baseUrl: string
...
export interface RobinTitleBarTheme {
  background: string
  foreground: string
}

export interface RobinWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface RobinApiRequest {
  path: string
  method?: string
  body?: unknown
  timeoutMs?: number
...
export interface RobinNotification {
  title?: string
  body?: string
  silent?: boolean
}

export interface RobinPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface RobinReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface RobinPreviewWatch {
  id: string
  path: string
}

export interface RobinReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface RobinReadDirResult {
  entries: RobinReadDirEntry[]
  error?: string
}

export interface RobinPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface RobinSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
