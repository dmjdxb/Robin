...
export const zh: Translations = {
  ...
  boot: {
    ...
    errors: {
      ...
      backgroundExitedDuringStartup: 'Robin 后台进程在启动期间退出。',
  ...
  notifications: {
    ...
    errors: {
      ...
      methodNotAllowed: '桌面后端拒绝了该请求(405 Method Not Allowed)。请尝试重启 Robin。',
  ...
  settings: {
    ...
    resetConfirm: '将所有设置恢复为 Robin 默认值？',
    exportFailed: '导出失败',
    resetFailed: '重置失败',
    nav: {
      gateway: '网关',
    ...
    appearance: {
      ...
      colorMode: '颜色模式',
      colorModeDesc: '选择固定模式，或让 Robin 跟随系统设置。',
    ...
    fieldDescriptions: {
      ...
      timezone: '当 Robin 需要本地时间上下文时使用。留空则使用系统时区。',
      ...
      file_read_max_chars: 'Robin 单次文件读取可读取的最大字符数。',
      'approvals.mode': 'Robin 如何处理需要显式审批的命令。',
      ...
      'agent.max_turns': 'Robin 停止一次运行前工具调用轮次的上限。'
    },
    about: {
      heading: 'Robin',
      ...
      automaticUpdatesDesc: 'Robin 会在后台自动检查更新,并在有可用更新时通知你。',
  ...
  commandCenter: {
    ...
    nav: {
      ...
      settings: { title: '设置', detail: '配置 Robin 桌面端' },
  ...
  messaging: {
    ...
    platformIntro: {
      ...
      signal: '在可访问的位置运行 signal-cli REST 桥接,然后把 Robin 指向该 URL 和已注册的电话号码。',
      whatsapp: '启动 Robin 自带的 WhatsApp 桥接,首次运行时扫描二维码,然后启用该平台。',
      bluebubbles: '在装有 iMessage 的 Mac 上运行 BlueBubbles Server,暴露其 API,然后用服务器密码把 Robin 指向该 URL。',
      ...
      weixin: '登录微信公众平台,复制 AppID 和 Token,并把消息回调 URL 指向 Robin。',
      qqbot: '在 QQ 开放平台(q.qq.com)注册一个应用,复制 App ID 和 Client Secret。',
      api_server: '把 Robin 暴露为兼容 OpenAI 的 API。设置一个鉴权密钥,然后把 Open WebUI / LobeChat 等指向 host:port。',
  ...
  profiles: {
    ...
    deleting: '删除中…',
    createDesc: '配置档案是相互独立的 Robin 环境:各自拥有独立的配置、技能和 SOUL.md。',
    nameLabel: '名称',
  ...
  cron: {
    ...
    emptyDescNew: '按 cron 表达式排程一个提示词。Robin 会运行它,并把结果发送到你选择的目的地。',
  ...
  composer: {
    message: '消息',
    placeholderStarting: '正在启动 Robin…',
    placeholderReconnecting: '正在重新连接 Robin…',
    ...
    effortLabel: '强度',
    effortTitle: '强度',
    effortHint: '更高的强度意味着更全面的回答，但耗时更长，并更快消耗你的用量。',
    queueMessage: '排队消息',
    stop: '停止',
    send: '发送',
    speaking: '讲话中',
    ...
    attachUrlDesc: 'Robin 将抓取该页面并作为本回合的上下文。',
