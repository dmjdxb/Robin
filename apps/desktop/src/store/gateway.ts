...
import { RobinGateway } from '@/hermes'
...
const isOpen = (gateway: RobinGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<RobinGateway | null>(null)
...
let primaryGateway: RobinGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: RobinGateway | null, profile = 'default'): void {
...
export function activeGateway(): RobinGateway | null {
...
function createSecondary(profile: string): Secondary {
  const gateway = new RobinGateway()
...
export async function ensureActiveGatewayOpen(): Promise<RobinGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
