...
describe('OAuth onboarding', () => {
  ...
  it('clears stale readiness errors after OAuth succeeds and model confirmation is shown', async () => {
    ...
    $desktopOnboarding.set(
      baseState({
        flow: {
          ...
          provider: provider('nous', 'Together AI'),
          start: {
        ...
        reason:
          'No access token found for Together AI login. setup.status reports configured credentials, but runtime resolution still failed.',
        requested: true
      })
    )
    ...
    if (state.flow.status === 'confirming_model') {
      expect(state.flow.label).toBe('Together AI')
