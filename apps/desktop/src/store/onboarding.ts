...
// mirroring robin/nous_subscription._GATEWAY_TOOL_LABELS so the GUI and
...
// When switching to EnergyIR auto-routes unconfigured tools through the Tool
...
function notifyGatewayTools(tools: string[] | undefined) {
  ...
  notify({
    durationMs: 8000,
    kind: 'info',
    message: `${list} now run through your provider subscription — no separate API keys needed.`,
