...
export type RobinConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
  ...
  /** Provider only: whether the current account is on the free tier. */
  free_tier?: boolean
  /** Provider only: paid models a free-tier user cannot select (shown disabled). */
...
export interface ToolProvider {
   ...
   *  CLI `robin tools` active-provider detection). */
  is_active: boolean
}
