#!/usr/bin/env python3
"""
Robin CLI - Interactive Terminal Interface

A beautiful command-line interface for the Robin, inspired by Claude Code.
...
from robin.fallback_config import get_fallback_chain
...
try:
    from robin.pt_input_extras import (
...
from robin.banner import _format_context_length, format_banner_version_label
...
from robin.browser_connect import (
    DEFAULT_BROWSER_CDP_URL,
    is_browser_debug_ready,
    manual_chrome_debug_command,
    try_launch_chrome_debug,
)
from robin.env_loader import load_hermes_dotenv
...
def load_cli_config() -> Dict[str, Any]:
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Robin, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Robin - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see robin/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
        ...
        try:
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and RobinCLI.__init__ checks "default" first.
    ...
    from robin.config import _expand_env_vars
    defaults = _expand_env_vars(defaults)

    # Apply terminal config to environment variables (so terminal_tool picks them up)
    terminal_config = defaults.get("terminal", {})
    
    # Normalize config key: the new config system (robin/config.py) and all
...
try:
    from robin.config import print_config_warnings
...
try:
    from robin.skin_engine import init_skin_from_config
...
def get_tool_definitions(*args, **kwargs):
    from robin.mcp_startup import wait_for_mcp_discovery
...
from robin.banner import build_welcome_banner
from robin.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from robin.callbacks import prompt_for_secret
...
def _prepare_deferred_agent_startup() -> None:
    ...
    try:
        from robin.plugins import discover_plugins
    ...
    try:
        from robin.mcp_startup import start_background_mcp_discovery
...
def _run_cleanup():
    ...
    try:
        from robin.plugins import invoke_hook as _invoke_hook
        _invoke_hook(
            "on_session_finalize",
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`robin.config.load_config` (the authoritative loader that
    ...
    if session_db is None:
        return
    try:
        from robin.config import load_config as _load_full_config
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`robin.config.load_config`. Honours ``auto_prune`` /
...
#   6. Default: assume dark (matches the legacy Robin assumption)
#
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from robin.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
...
class _SkinAwareAnsi:
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from robin.skin_engine import get_active_skin
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Robin output it's common to display cron schedules verbatim.
    #
...
# ASCII Art - Robin Caduceus (compact, fits in left panel)
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        ...
        agent_name = _skin.get_branding("agent_name", "Robin") if _skin else "Robin"
        line1 = f"{agent_name} - AI Agent Framework"
        tiny_line = agent_name

    if os.environ.get("HERMES_FAST_STARTUP_BANNER") == "1":
        from robin import __release_date__ as _release_date
        from robin import __version__ as _version

        version_line = f"Robin v{_version} ({_release_date})"
    else:
        version_line = format_banner_version_label()

    w = min(shutil.get_terminal_size().columns - 2, 88)
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- EnergyIR[/]\n"
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from robin.plugins import get_plugin_commands
...
class RobinCLI:
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from robin.runtime_provider import _auto_detect_local_model
        ...
        # by `hermes chat --ignore-rules` in robin/main.py. When true we
        ...
        # it's shared across all Robin processes for this HERMES_HOME.
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from robin.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'Robin'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from robin.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from robin.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from robin.models import normalize_opencode_model_id, opencode_model_api_mode
        ...
        if self._model_is_default:
            ...
            try:
                from robin.codex_models import get_codex_model_ids
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from robin.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "⚕ Robin")
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - RobinCLI._status_bar_display_width(label)
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        """
        from robin.runtime_provider import (
        ...
        if not self.model and resolved_provider:
            try:
                from robin.models import get_default_model_for_provider
    ...
    def _resolve_turn_agent_config(self, user_message: str) -> dict:
        ...
        """
        from robin.models import resolve_fast_mode_overrides
    ...
    def _show_security_advisories(self):
        ...
        """
        try:
            from robin.security_advisories import (
                detect_compromised,
                startup_banner,
            )
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Robin needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        # Warn if the configured model is a EnergyIR Robin LLM (not agentic)
        from robin.model_switch import is_nous_hermes_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_hermes_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  EnergyIR Robin 3 & 4 models are NOT agentic and are not "
                "designed for use with Robin.[/]"
            )
            self._console_print(
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ Robin: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ Robin: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        """
        from robin.clipboard import save_clipboard_image
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Robin config/state.
    ...
    def _handle_paste_command(self):
        ...
        from robin.clipboard import has_clipboard_image
    ...
    def _fast_command_available(self) -> bool:
        try:
            from robin.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def show_help(self):
        ...
        from robin.commands import COMMANDS_BY_CATEGORY

        try:
            from robin.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Robin!{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        from io import StringIO
        from robin.tools_config import tools_disable_enable_command
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in robin/colors.py
        ...
        from robin.tools_config import _get_platform_tools
        from robin.config import load_config
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from robin.main import _relative_time
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if target.isdigit():
            ...
            from robin.main import _resolve_session_by_name_or_id
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from robin.curses_ui import curses_single_select
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        # Copilot, and EnergyIR-enforced caps win over the raw models.dev entry
        # (e.g. gpt-5.5 is 1.05M on openai but 272K on Codex OAuth).
        mi = result.model_info
        try:
            from robin.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            if not model_list:
                try:
                    from robin.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from robin.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from robin.model_switch import switch_model, parse_model_flags
        from robin.providers import get_label
        ...
        if force_refresh:
            try:
                from robin.models import clear_provider_models_cache
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from robin.commands import resolve_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to robin.curator so the CLI and the `hermes curator`
        ...
        try:
            from robin.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_kanban_command(self, cmd: str):
        ...
        """
        from robin.kanban import run_slash
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to robin.skills_hub."""
        from robin.skills_hub import handle_skills_slash
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in robin/commands.py instead of touching every dispatch site.
        from robin.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            if self._app:
                ...
                try:
                    from robin.tips import get_random_tip
                    _tip = get_random_tip()
                    try:
                        from robin.skin_engine import get_active_skin
            ...
            from robin.config import reload_env
            count = reload_env()
            ...
            try:
                from robin.plugins import get_plugin_manager
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from robin.commands import COMMANDS
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Robin-managed isolated debug "
    ...
    def _get_goal_manager(self):
        ...
        """
        try:
            from robin.goals import GoalManager
            from robin.config import load_config
        except Exception as exc:
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Robin keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from robin.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        """
        from robin.config import load_config
        from robin.colors import Colors as _Colors
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Robin is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Robin is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Robin is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from robin.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        from robin.partial_compress import (
    ...
    def _handle_debug_command(self):
        ...
        from robin.debug import run_debug_share
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Robin to the latest version.
        ...
        when cancelled.
        """
        from robin.config import is_managed, format_managed_message

        if is_managed():
            print(f"  ✗ {format_managed_message('update Robin')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Robin"),
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from robin.config import get_config_path as _get_config_path
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            try:
                from robin.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from robin.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from robin.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Robin! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Robin! Type your message or /help for commands."
        ...
        try:
            from robin.model_switch import prewarm_picker_cache_async
        ...
        # after an OpenClaw→Robin migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from robin.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in robin.pt_input_extras
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Robin binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from robin.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Robin")
        ...
        # Dynamic prompt: shows Robin symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Robin needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Robin needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Robin needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = RobinCLI._compute_model_picker_viewport(
        ...
        if getattr(self, '_pending_relaunch', None):
            from robin.relaunch import relaunch
...
def _run_kanban_goal_loop_q(cli: "RobinCLI", first_response: str) -> None:
    ...
    from robin import kanban_db as _kb
    from robin.goals import run_kanban_goal_loop as _run_loop, DEFAULT_MAX_TURNS as _DEF_TURNS
...
def main(
    ...
    """
    Robin CLI - Interactive AI Assistant
    ...
    try:
        from robin.stdio import configure_windows_stdio
    ...
    if gateway:
        ...
        print("Starting Robin Gateway (messaging platforms)...")
    ...
    # mode registers its own inside RobinCLI.run(), but `-q` runs
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "Robin" box, no tool-gen
