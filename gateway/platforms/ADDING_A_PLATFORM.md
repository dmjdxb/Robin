...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `robin/config.py` so the setup
...
## 12. Status Display (`robin/status.py`)
...
## 13. Gateway Setup Wizard (`robin/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ robin/ toolsets.py \
  --include="*.py" -l | sort -u
