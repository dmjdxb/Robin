...
version: 0.1.0
author: Matt Maximo (MattMaximo), Robin
license: MIT
...
# Grok Build CLI — Robin Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Robin terminal. Grok
...
1. Prepare stable input files first with Robin tools (`read_file`,
...
2. **Don't conflate Robin' xAI auth with the `grok` CLI's auth.** Robin'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Robin' xAI auth carries
   over.
