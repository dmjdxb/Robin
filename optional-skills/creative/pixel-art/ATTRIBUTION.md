...
- `generate_video()` split: the Pollinations text-to-image call was removed
  (Robin uses its own `image_generate` + `pixel_art()` pipeline for base
...
- Pollinations text-to-image generation (`pixelart_image.py`,
  `generate_base()` in `pixelart_video.py`) — Robin has `image_generate`
...
- Expansion with palettes + video: Robin contributors
