...
│   │   └── skills_sync.py
│   └── 02-reconcile-profiles          ← robin.container_boot
...
| `docker/cont-init.d/02-reconcile-profiles` | Calls `robin.container_boot` on every boot to restore profile gateway slots from the persistent volume. |
...
| `robin/service_manager.py` | `S6ServiceManager`: `register_profile_gateway`, `unregister_profile_gateway`, `start/stop/restart/is_running`, `list_profile_gateways`. |
| `robin/container_boot.py` | `reconcile_profile_gateways()` — walks persistent profiles, regenerates s6 slots, emits `container-boot.log`. |
| `robin/gateway.py::_dispatch_via_service_manager_if_s6` | Intercepts `hermes gateway start/stop/restart` and routes to s6 when running in a container. |
...
Edit `S6ServiceManager._render_run_script` in `robin/service_manager.py`. The function is also called by `robin/container_boot.py::_register_service` during boot reconciliation, so it's the single source of truth. Update the corresponding assertion in `tests/robin/test_service_manager.py::test_s6_register_creates_service_dir_and_triggers_scan`.
