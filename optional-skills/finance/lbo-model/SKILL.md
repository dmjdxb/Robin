...
version: 1.0.0
author: Anthropic (adapted by EnergyIR)
license: Apache-2.0
...
Many passages below say "use the S&P Kensho MCP / Daloopa MCP / FactSet MCP". Those are commercial financial-data MCPs from the original Cowork plugin context. In Robin:

- **If you have any structured financial-data MCP configured** (Robin supports MCP — see `native-mcp` skill), prefer it for point-in-time comps, precedent transactions, and filings.
