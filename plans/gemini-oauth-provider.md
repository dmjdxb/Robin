...
## Client ID
- Need to register a "Desktop app" OAuth client on a EnergyIR GCP project
...
2. `robin/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `robin/models.py` — Add Gemini model catalog
4. `robin/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `robin/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `robin/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- EnergyIR GCP project with Desktop OAuth client registered
...
- hermes-agent Copilot OAuth: `robin/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
