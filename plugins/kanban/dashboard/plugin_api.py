...
``robin.kanban_db`` or a direct SQL query. Writes use the same code
...
the in-browser PTY bridge in ``robin/web_server.py``.
...
from robin import kanban_db
from robin import kanban_diagnostics as kd
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    WS auth gate (``robin.web_server._ws_auth_ok``).
    ...
    """
    try:
        from robin import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``robin.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
    from robin import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from robin.kanban_diagnostics import SEVERITY_ORDER
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from robin.kanban import _check_dispatcher_presence
...
# spawn failures, stuck-blocked). See robin.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def specify_task_endpoint(
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        ...
        from robin import kanban_specify  # noqa: WPS433 (intentional)
...
def _active_profile_name() -> str:
    """Return the current Robin profile name for notify-sub ownership."""
    try:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    """
    try:
        from robin import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        from robin import kanban_decompose  # noqa: WPS433 (intentional)
