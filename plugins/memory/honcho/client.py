...
class HonchoClientConfig:
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Robin session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — Robin session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Robin session_id (new Honcho session each run)
