"""langfuse — Robin plugin for Langfuse observability.

Traces Robin conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Robin plugin system — standalone plugins only
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Robin plugin system —
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Robin branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Robin also has a turn-scoped
    ...
    # messages is a list only for legacy Robin branches that fired
    # pre_llm_call with API messages directly. Current Robin fires
...
def register(ctx) -> None:
    ...
    # Robin versions.  pre_api_request / post_api_request fire per API
