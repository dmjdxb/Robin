...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Robin manages."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Robin session")
        ...
        @tree.command(name="status", description="Show Robin session status")
        ...
        @tree.command(name="stop", description="Stop the running Robin agent")
        ...
        @tree.command(name="update", description="Update Robin to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Robin gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Robin session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Robin in the thread",
        ...
        # robin/commands.py automatically appear as Discord slash
        ...
        try:
            from robin.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from robin.commands import _iter_plugin_command_entries
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from robin.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Robin session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Robin: **{name}**"
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        thread_name = content[:80] if content else "Robin"
        if len(content) > 80:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Thread created by Robin: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Robin handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``robin.gateway.get_env_value`` at call time (not via
    ...
    """
    import robin.gateway as gateway_mod
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # robin/setup.py::_setup_discord function.  Same shape as Teams.
