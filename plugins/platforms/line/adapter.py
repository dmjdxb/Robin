"""
LINE Messaging API platform adapter for Robin.
...
to Robin. It deliberately ports the *strongest* idea from each into
...
def interactive_setup() -> None:
    ...
    one optional public URL. Writes to ``~/.hermes/.env`` via ``robin.config``.
    """
    print()
    ...
    try:
        from robin.config import get_env_var, set_env_var
    except ImportError:
        print("robin.config not available; set LINE_* vars manually in ~/.hermes/.env")
        return
