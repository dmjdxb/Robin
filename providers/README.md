...
Registry and ABC for every inference provider Robin knows about.
...
- `robin/auth.py` extends `PROVIDER_REGISTRY` with every api-key
  profile it sees (skipping `copilot`, `kimi-coding`, `kimi-coding-cn`,
  `zai`, `openrouter`, `custom` — those need bespoke token resolution).
- `robin/models.py` extends `CANONICAL_PROVIDERS` and calls
  `profile.fetch_models()` inside `provider_model_ids()`.
- `robin/doctor.py` adds a `/models` health check for each
  `auth_type="api_key"` profile.
- `robin/config.py` injects every `env_var` into
  `OPTIONAL_ENV_VARS` so the setup wizard knows about it.
- `robin/runtime_provider.py` reads `profile.api_mode` as a fallback
