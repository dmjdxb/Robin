...
Pure display functions with no RobinCLI state dependency.
"""
...
from robin import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
_UPSTREAM_REPO_URL = "https://github.com/dmjdxb/Robin.git"
...
def check_for_updates() -> Optional[int]:
    """Check whether a Robin update is available.
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Robin git checkout, or None if this isn't a git install.
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``robin/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from robin.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/dmjdxb/Robin/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Robin checkout. Cached per-process. Release URL always points at the
    canonical dmjdxb/Robin repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Robin v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]EnergyIR[/]")
    ...
    try:
        from robin.codex_runtime_switch import get_current_runtime
        from robin.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from robin.config import get_managed_update_command, recommended_update_command
            if behind > 0:
