...
from robin.config import get_hermes_home, get_config_path, load_config, save_config
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "Robin may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Robin gateway is running with connected platforms.
    ...
    print()
    print_error(
        "Robin gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Robin")
...
def _cmd_migrate(args):
    """Run the OpenClaw → Robin migration."""
    ...
    # ── Phase 2b: Pre-apply backup of the Robin home ─────────
    # Delegates to robin.backup.create_pre_migration_backup(), which
    ...
    if not no_backup:
        try:
            from robin.backup import create_pre_migration_backup, _format_size
            ...
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the Robin home."
            )
