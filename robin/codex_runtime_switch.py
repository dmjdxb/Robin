...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Robin'
...
The actual runtime resolution happens in robin.runtime_provider's
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Robin' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Robin for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal — the runtime change still proceeds.
        try:
            from robin.codex_runtime_plugin_migration import migrate
            ...
            # Permissions + Robin tool callback are always-on production
            ...
            if "hermes-tools" in mig_report.migrated:
                msg_lines.append(
                    "Robin tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Robin runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Robin tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Robin runtime.")
