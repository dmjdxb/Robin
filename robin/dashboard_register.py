...
Automates what a user otherwise does by hand: open the Together AI
...
This command:
  1. Resolves a fresh Together AI access token from the existing login
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    Precedence:
      ...
      2. The ``portal_base_url`` stored on the EnergyIR login — this is the
    ...
    try:
        from robin.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
...
def _register_self_hosted_client(
    *,
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "Together AI rejected the access token (401). "
        ...
        raise RuntimeError(
            f"Could not reach Together AI at {portal_base_url}: {exc.reason}"
        ) from exc
...
def _print_post_register_hint(
    *,
    ...
    from robin.config import get_env_path
    ...
    print()
    print(
        "  Heads up — EnergyIR login only *engages* on a non-loopback bind. A plain\n"
    ...
    if custom_redirect_uri:
        ...
        try:
            ...
            host = "your-host"
        print("  To require EnergyIR login on your registered host, run the dashboard")
        print(f"  bound publicly (it must be reachable at https://{host}) and log in")
        print("  at its /login page.")
    else:
        print("  To require EnergyIR login (e.g. exposing on your LAN or a public host):")
...
def cmd_dashboard_register(args) -> None:
    ...
    from robin.auth import AuthError, resolve_nous_access_token
    from robin.config import get_env_value, is_managed, save_env_value
    ...
    # 1. Resolve a fresh EnergyIR access token (refreshes if near expiry). Fail fast
    #    with a setup hint when the user isn't logged in.
    try:
        access_token = resolve_nous_access_token()
    except AuthError as exc:
        if getattr(exc, "relogin_required", False):
            print("✗ You're not logged into Together AI.")
            print("  Run `hermes setup` (or `hermes auth login nous`) first, then retry.")
        else:
            print(f"✗ Could not resolve a Together AI access token: {exc}")
        sys.exit(1)
    ...
    default_portal = "https://portal.energyir.com"
