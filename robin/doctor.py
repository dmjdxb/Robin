...
from robin.config import get_project_root, get_hermes_home, get_env_path
...
from robin.models import _HERMES_USER_AGENT
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized in {"google / gemini", "gemini"}:
        try:
            from robin.auth import get_gemini_oauth_auth_status
            return bool((get_gemini_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "minimax":
        try:
            from robin.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches robin.__version__.
    ...
    """
    try:
        from robin import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != robin/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'hermes update', or set "
            "robin/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from robin.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # Detect drift between pyproject.toml and robin/__init__.py versions
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from robin.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from robin.auth import PROVIDER_REGISTRY, get_auth_status
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from robin.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from robin.config import check_config_version, migrate_config
        ...
        try:
            import yaml
            from robin.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
    ...
    try:
        from robin.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
