"""Helpers for loading Robin .env files consistently across entrypoints."""
...
# several hot modules (cli.py, robin/main.py, run_agent.py,
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``robin.config._sanitize_env_lines`` which
    already knows all valid Robin env-var names and can split
    concatenated lines correctly.
    """
    if not path.exists():
        return
    try:
        from robin.config import _sanitize_env_lines
    except ImportError:
...
def load_hermes_dotenv(
    *,
    ...
    """Load Robin environment files with user config taking precedence.
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    locate the access token) but BEFORE the rest of Robin reads
    ``os.environ`` for credentials.  Any failure here is logged and
    swallowed — external secret sources must never block startup.

    Idempotent within a process: subsequent calls for the same
    ``home_path`` are no-ops.  ``load_hermes_dotenv()`` runs at import
    time from several hot modules (cli.py, robin/main.py,
