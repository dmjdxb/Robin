...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m robin.main gateway run --replace`` invocation) and
...
_TASK_DESCRIPTION = "Robin Gateway - Messaging Platform Integration"
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Robin profile."""
    from robin.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "robin.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def get_task_name() -> str:
    ...
    """
    _assert_windows()
    # Local import to avoid circular module initialization during robin boot.
    from robin.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Robin installs stay self-contained).
    """
    _assert_windows()
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - invokes ``pythonw -m robin.main [--profile X] gateway run``
    ...
    prog_args = [pythonw_path, "-m", "robin.main"]
    if profile_arg:
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Robin builds and failed
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m robin.main gateway run``
...
def _prompt_install_choices(
    ...
    from robin.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    from robin.gateway import find_gateway_pids, _profile_arg
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Robin gateway install prompt.")
                if start_now:
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Robin gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: hermes gateway status")
                    return
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from robin.gateway import kill_gateway_processes, _get_restart_drain_timeout
