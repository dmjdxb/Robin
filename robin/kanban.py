"""CLI for the Robin Kanban board — ``hermes kanban …`` subcommand.
...
from robin import kanban_swarm as ks
from robin.profiles import get_active_profile_name, get_profile_dir, seed_profile_skills
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            "Durable SQLite-backed task board shared across Robin profiles. "
            "Tasks are claimed atomically, can depend on other tasks, and "
            "are executed by a named profile in an isolated workspace. "
            "See https://robin.energyir.com/docs/user-guide/features/kanban "
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real Robin profile (i.e. one the
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from robin import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from robin import kanban_decompose as decomp
