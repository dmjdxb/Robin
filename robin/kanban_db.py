...
``<root>`` is the **shared Robin root** (the parent of any active
...
the profile's Robin home but are otherwise isolated: a worker spawned
...
def kanban_home() -> Path:
    """Return the shared Robin root that anchors the kanban board.
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Robin treats as user-controlled-but-trusted on the
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from robin.profiles import normalize_profile_name
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Robin' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Robin-managed storage. A board ``default_workdir`` pointing at a
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Robin
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Robin profile.
    ...
    if not rows:
        return False
    try:
        from robin.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def dispatch_once(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from robin.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Robin profile.
        # `_default_spawn` invokes ``hermes -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Robin
...
def _module_hermes_argv() -> list[str]:
    """Return the interpreter-bound Robin CLI invocation."""
    # ``robin.main`` is the console-script target declared in
...
def _absolute_hermes_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Robin shim."""
...
def _hermes_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Robin executable path.
...
def _resolve_hermes_argv() -> list[str]:
    ...
    3. ``sys.executable -m robin.main`` — fallback for setups where
       Robin is launched from a venv and the ``hermes`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_hermes_bin`` for the same reason. Kept
    local (not imported from gateway) because ``robin`` sits below
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install Robin or activate its venv before running the kanban dispatcher."
        )
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Robin root exists

    Reads profile paths directly so this module has no import dependency on
    ``robin.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from hermes_constants import get_default_hermes_root
