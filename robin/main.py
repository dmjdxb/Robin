...
# that run *before* robin.config is importable. Mirrors the explicit
...
def _apply_profile_override() -> None:
    ...
    # Mirrors robin.profiles._PROFILE_ID_RE so we never call
    ...
    if profile_name is not None:
        try:
            from robin.profiles import resolve_profile_env
...
from robin import __version__, __release_date__
...
def _has_any_provider_configured() -> bool:
    ...
    from robin.config import DEFAULT_CONFIG
...
def _find_bundled_tui(robin_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if robin_dir is None:
        robin_dir = Path(__file__).parent
    bundled = robin_dir / "tui_dist" / "entry.js"
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        if not path and bin == "node":
            try:
                from robin.dep_ensure import ensure_dependency
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from robin.oneshot import _normalize_toolsets
...
def _pin_kanban_board_env() -> None:
    ...
    if os.environ.get("HERMES_KANBAN_BOARD"):
        return
    try:
        from robin.kanban_db import get_current_board
...
def cmd_chat(args):
    ...
    try:
        from robin.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from robin.config import load_config as _load_config_for_xai_check
    ...
    if _termux_should_prefetch_update_check():
        try:
            from robin.banner import prefetch_update_check
...
def cmd_gateway(args):
    ...
    from robin.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from robin.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from robin.config import get_env_value, save_env_value
...
def cmd_setup(args):
    ...
    from robin.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from robin.config import stamp_install_method
    from robin.dep_ensure import ensure_dependency
...
def select_provider_and_model(args=None):
    ...
    from robin.providers import resolve_provider_full
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from robin.config import read_raw_config
    ...
    # PROVIDER_GROUPS in robin/models.py). A multi-member group shows one
...
def _clear_stale_openai_base_url():
    ...
    """
    from robin.config import get_env_value, save_env_value, load_config
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`robin.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from robin.plugins import get_plugin_auxiliary_tasks
...
def _aux_select_for_task(task: str) -> None:
    ...
    """
    from robin.config import load_config
    from robin.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from robin.auth import _prompt_model_selection
    from robin.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from robin.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from robin.config import get_env_value
    ...
    from robin.models import model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from robin.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier:
        try:
            from robin.nous_account import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from robin.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    if not _codex_token:
        try:
            from robin.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from robin.models import _PROVIDER_MODELS
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from robin.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from robin.auth import _save_model_choice, deactivate_provider
    from robin.config import get_env_value, load_config, save_config
    from robin.secret_prompt import masked_secret_prompt
    ...
    from robin.models import probe_api_models
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from robin.runtime_provider import _detect_api_mode_for_url
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from robin.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from robin.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from robin import azure_detect
...
def _remove_custom_provider(config):
    ...
    try:
        from robin.curses_ui import curses_radiolist
...
# downstream call sites read `robin.main._PROVIDER_MODELS` directly,
...
def _model_flow_copilot(config, current_model=""):
    ...
    from robin.config import save_env_value, load_config, save_config
    from robin.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from robin.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from robin.copilot_auth import validate_copilot_token
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from robin.env_loader import format_secret_source_suffix
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    """
    from robin.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from robin.config import save_env_value
    from robin.secret_prompt import masked_secret_prompt
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from robin.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "lmstudio":
        ...
        from robin.models import fetch_lmstudio_models
        ...
        from robin.models import fetch_ollama_cloud_models
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from robin.auth import get_anthropic_key
...
def cmd_logout(args):
    ...
    from robin.auth import logout_command
...
def cmd_auth(args):
    ...
    from robin.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from robin.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from robin.cron import cron_command
...
def cmd_webhook(args):
    ...
    from robin.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from robin.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from robin.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from robin.hooks import hooks_command
...
def cmd_security(args):
    ...
    if sub in ("audit", None):
        from robin.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from robin.dump import run_dump
...
def cmd_debug(args):
    ...
    from robin.debug import run_debug
...
def cmd_config(args):
    ...
    from robin.config import config_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        ...
        from robin.backup import run_backup
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        ...
        from robin.config import recommended_update_command
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``robin/web_dist/`` (per vite.config.ts
    outDir: "../robin/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "robin" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``hermes`` launcher present but ``robin`` not importable.
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # build path usable on a stock `py -m robin.main web` invocation.
...
def _find_stale_dashboard_pids(
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    patterns = [
        "hermes dashboard",
        "robin.main dashboard",
        "robin/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # with `robin.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    ...
    print(
        "  Docs:         https://robin.energyir.com/docs/user-guide/features/curator"
    )
...
def _update_via_zip(args):
    ...
    if branch != "main":
        ...
        sys.exit(1)
    zip_url = (
        f"https://github.com/dmjdxb/Robin/archive/refs/heads/{branch}.zip"
    )
    ...
    from robin.managed_uv import ensure_uv, update_managed_uv
...
OFFICIAL_REPO_URLS = {
    "https://github.com/dmjdxb/Robin.git",
    "git@github.com:dmjdxb/Robin.git",
    "https://github.com/dmjdxb/Robin",
    "git@github.com:dmjdxb/Robin",
}
OFFICIAL_REPO_URL = "https://github.com/dmjdxb/Robin.git"
    ...
    if not has_upstream:
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/dmjdxb/Robin.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/dmjdxb/Robin.git' to add later."
            )
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from robin.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    """
    from robin.managed_uv import resolve_uv
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # robin.config.get_hermes_home to simulate setup failure.
        from robin.config import get_hermes_home as _get_hermes_home
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    if method == "docker":
        ...
        from robin.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from robin.config import recommended_update_command
        from robin.banner import check_via_pypi
...
def _run_pre_update_backup(args) -> None:
    ...
    try:
        from robin.backup import create_pre_update_backup
    except Exception as exc:
        print(
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        try:
            from robin.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in robin/config.py bricked
        ...
        if (desktop_dir / "package.json").exists() and shutil.which("npm") and has_desktop_app:
            ...
            _desktop_build_cmd = [sys.executable, "-m", "robin.main", "desktop", "--build-only"]
        ...
        try:
            from robin.backup import restore_cron_jobs_if_emptied
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from robin.tools_config import install_cua_driver
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        from robin import profiles as _profiles_mod
        ...
        from robin import profile_describer as _pd
        ...
        from robin.profiles import profile_exists
        ...
        from robin.profiles import rename_profile
        ...
        from robin.profiles import export_profile
        ...
        from robin.profiles import import_profile
        ...
        import tempfile
        from robin.profile_distribution import (
            plan_install,
        ...
        from robin.profiles import get_profile_dir, normalize_profile_name
        ...
        from robin.profile_distribution import describe_distribution, DistributionError
...
def _render_distribution_plan(plan) -> None:
    ...
    from robin.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
...
def cmd_dashboard(args):
    ...
    if "HERMES_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            ...
            else PROJECT_ROOT / "robin" / "web_dist"
        )
    ...
    from robin.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from robin.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_completion(args, parser=None):
    ...
    from robin.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from robin.prompt_size import cmd_prompt_size as _impl
...
# the top-level flags declared in ``robin/_parser.py``.
#
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from robin._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from robin.oneshot import run_oneshot
...
def main():
    ...
    # =========================================================================
    from robin.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://robin.energyir.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://robin.energyir.com/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from robin import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from robin.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    from robin.send_cmd import register_send_subparser
    ...
    # =========================================================================
    from robin.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from robin.kanban import build_parser as _build_kanban_parser
...
""",
    ...
    from robin.checkpoints import register_cli as _register_checkpoints_cli
    ...
    def cmd_pairing(args):
        from robin.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from robin.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from robin.skills_hub import skills_command
    ...
    from robin.bundles import register_cli as _bundles_register, bundles_command
    ...
    def cmd_plugins(args):
        from robin.plugins_cmd import plugins_command
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from robin.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from robin.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from robin.memory_setup import memory_command
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            ...
            _require_tty("tools")
            from robin.tools_config import tools_command
    ...
    def cmd_mcp(args):
        from robin.mcp_config import mcp_command
    ...
    def cmd_claw(args):
        from robin.claw import claw_command
...
""",
    ...
    from robin.config import get_container_exec_info
