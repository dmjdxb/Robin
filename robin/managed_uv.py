...
Robin owns its own uv binary at ``$HERMES_HOME/bin/uv`` (or ``uv.exe`` on
...
def managed_uv_path() -> Path:
    """Return the path where Robin keeps *its* uv binary.
...
class _UvResult(str):
    ...
    the call site from the *old*, already-imported ``robin.main`` against
