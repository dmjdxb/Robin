...
- ``robin.providers``        -- canonical provider identity + overlays
- ``robin.model_normalize``  -- per-provider name formatting
...
#   NousResearch/Robin-3-Llama-3.1-70B, hermes-4-405b, openrouter/hermes3:70b
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from robin.model_switch import
...
def resolve_display_context_length(
    ...
    about Codex OAuth, Copilot, EnergyIR, and falls back to models.dev for the
    rest.
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            ...
            try:
                from robin.config import validate_config_structure
        ...
        from robin.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from robin.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from robin.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in robin.runtime_provider.resolve_runtime_provider;
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from robin.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from robin/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from robin.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # --- 1. Check Robin-mapped providers ---
    from robin.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from robin.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if not has_creds:
            try:
                from robin.auth import _load_auth_store
    ...
    # --- 2. Check Robin-only providers (nous, openai-codex, copilot, opencode-go) ---
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            # EnergyIR serves a large live /v1/models catalog (vendor-prefixed
            ...
            try:
                ...
                from robin.auth import get_provider_auth_state as _nous_state
    ...
    # with `hermes model`).
    try:
        from robin.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`robin.models.fetch_openrouter_models`, which filters the
    ...
    """
    from robin.models import fetch_openrouter_models
