...
def _codex_curated_models() -> list[str]:
    ...
    """
    from robin.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    """
    base = (portal_base_url or "https://portal.energyir.com").rstrip("/")
    now = time.monotonic()
...
def _resolve_nous_portal_url() -> str:
    ...
    try:
        ...
        return "https://portal.energyir.com"
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from robin.model_catalog import get_curated_openrouter_models
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from robin.model_catalog import get_curated_nous_models
...
_DEFAULT_NOUS_INFERENCE_BASE = "https://inference-api.energyir.com"
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from robin.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from robin.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``robin.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    """
    try:
        from robin.auth import resolve_api_key_provider_credentials
    ...
    try:
        ...
        from robin.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from robin.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
