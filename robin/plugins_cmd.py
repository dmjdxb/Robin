...
def _resolve_git_executable() -> Optional[str]:
    ...
    Matches other Robin subprocess resolution: :func:`shutil.which` first,
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from robin.config import get_env_value, save_env_value  # noqa: F811
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                ...
                raise PluginOperationError(
                    ...
                    f"Run {recommended_update_command()} to update Robin.",
                ) from None
...
def cmd_install(
    ...
    if not (target / "plugin.yaml").exists() and not (target / "plugin.yml").exists() and not (
        ...
        console.print(
            ...
            f"or __init__.py. It may not be a valid Robin plugin.",
        )
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    plugin-registered engines (third-party engines installed as Robin
    ...
    try:
        from robin.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def _run_composite_ui(curses, plugin_names, plugin_labels, plugin_selected,
    ...
    from robin.curses_ui import flush_stdin
