"""Profile distributions — shareable, packaged Robin profiles via git.

A distribution is a Robin profile published as a git repository (or
...
def check_hermes_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Robin {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Robin distribution.",
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Robin profile distribution."
            )
...
def plan_install(
    ...
    from robin import __version__ as hermes_version
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Robin distribution."
        )
