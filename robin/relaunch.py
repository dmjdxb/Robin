"""
Unified self-relaunch for Robin CLI.
...
def resolve_hermes_bin() -> Optional[str]:
    ...
    Priority:
      ...
      3. ``None`` → caller should fall back to ``python -m robin.main``.
    ...
    ``hermes.exe`` on PATH or the ``sys.executable -m robin.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    if bin_path:
        ...
        argv = [sys.executable, "-m", "robin.main"]
...
def relaunch(
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    ``python -m robin.main``) or a ``.cmd`` batch file, and both
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            print(
                f"\nRobin relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run robin.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
