...
the existing module-level functions in robin.gateway and
robin.gateway_windows directly. This protocol is a thin facade
...
def _s6_running() -> bool:
    ...
    detection always fails. Since every Robin runtime call inside the
...
# in ``robin.gateway`` (systemd/launchd) and ``robin.gateway_windows``
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in robin.gateway.
    ...
    def start(self, name: str) -> None:
        from robin.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from robin.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from robin.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from robin.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in robin.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from robin.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from robin.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from robin.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from robin.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``robin.gateway_windows`` (Scheduled Task /
    ...
    def is_running(self, name: str) -> bool:
        from robin import gateway_windows
        from robin.gateway import find_gateway_pids
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    (the ``hermes`` user — UID 10000 — running every Robin runtime
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        ``robin.gateway`` maps that empty string to the
        ...
        ``robin.profiles._allocate_gateway_port``). PR #30136
