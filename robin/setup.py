...
_DOCS_BASE = "https://robin.energyir.com/docs"
...
from robin.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from robin.secret_prompt import masked_secret_prompt  # noqa: E402
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from robin.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    from robin.main import select_provider_and_model
    try:
...
def _setup_telegram_auto_result():
    ...
    try:
        from robin.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
...
def _setup_slack():
    ...
    print()
    print_info("   Full guide: https://robin.energyir.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    ...
    """
    try:
        from robin.slack_cli import _build_full_manifest
...
def _setup_qqbot():
    ...
    from robin.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://robin.energyir.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://robin.energyir.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
...
def setup_gateway(config: dict):
    ...
    from robin.gateway import _all_platforms, _platform_status, _configure_platform
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``robin.auth`` — lists every supported
    ...
    """
    try:
        from robin.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from robin.config
    ...
    if section_key == "model":
        ...
        from robin.gateway import _all_platforms, _platform_status
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    try:
        from robin.main import _model_flow_nous
...
def run_setup_wizard(args):
    ...
    """
    from robin.config import is_managed, managed_error
    if is_managed():
    ...
    print(
        color(
            "│             ⚕ Robin Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Robin installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
