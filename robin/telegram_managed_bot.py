...
manual BotFather token copy-paste. Robin talks only to the EnergyIR onboarding
...
# Default pairing API base URL (EnergyIR-hosted Cloudflare Worker).
# Override for PoC/staging with TELEGRAM_ONBOARDING_URL.
DEFAULT_API_URL = "https://setup.robin.energyir.com"
TELEGRAM_ONBOARDING_URL_ENV = "TELEGRAM_ONBOARDING_URL"

# The EnergyIR-hosted manager bot username (without @). The backend returns the
# actual deep link, so this is only used by local helpers/tests.
DEFAULT_MANAGER_BOT = "RobinSetupBot"

DEFAULT_BOT_NAME = "Robin"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Robin Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Robin Telegram onboarding service.")
