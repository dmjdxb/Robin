...
def remove_wrapper_script():
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains robin reference)
                content = wrapper.read_text()
                if 'robin' in content or 'hermes-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    try:
        from robin.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from robin.gateway import get_launchd_plist_path
...
def _hermes_path_markers(hermes_home: Path) -> list[str]:
    """Path-entry substrings that identify Robin-owned User-PATH entries."""
...
def remove_path_from_windows_registry(hermes_home: Path) -> list[str]:
    """Strip Robin-owned entries from User-scope PATH in the registry.
...
def _uninstall_profile(profile) -> None:
    ...
    #    Use `python -m robin.main` so we don't depend on a `hermes`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "robin.main", "--profile", name]
...
def run_uninstall(args):
    ...
    if _is_windows():
        ...
        if removed_path_entries:
            ...
            log_info("No Robin-owned PATH entries in User environment")
        ...
        if removed_env:
            ...
            log_info("No Robin-set User env vars to remove")
    ...
    if removed_node_links:
        ...
        log_info("No Robin-managed node/npm/npx symlinks found")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://robin.energyir.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://robin.energyir.com/install.sh | bash", Colors.DIM))
        print()
