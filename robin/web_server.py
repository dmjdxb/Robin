...
Usage:
    python -m robin.main web          # Start on http://127.0.0.1:9119
    python -m robin.main web --port 8080
"""
...
app = FastAPI(title="Robin", version=__version__, lifespan=_lifespan)
...
#
# This list is defined in ``robin.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from robin.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from robin.dashboard_auth.middleware import gated_auth_middleware
...
async def get_status():
    ...
    # SPA's StatusPage can show "OAuth gate ON via EnergyIR" or
    # "loopback only — no auth gate" with no extra round trips.
    auth_required = bool(getattr(app.state, "auth_required", False))
    auth_providers: list[str] = []
    try:
        from robin.dashboard_auth import list_providers as _list_providers
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from robin.auth import get_nous_auth_status
    ...
    features = []
    try:
        from robin.nous_subscription import get_nous_subscription_features
    ...
    return {
        ...
        "subscription_url": "https://portal.energyir.com/manage-subscription",
        "features": features,
    }
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from robin.debug import build_debug_share
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    Uses the running interpreter's ``robin.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "robin.main", *subcommand]
...
# in robin/config.py — listed here for deterministic ordering in the UI.
...
def get_recommended_default_model(provider: str = ""):
    ...
    if slug == "nous":
        try:
            ...
            from robin.auth import get_provider_auth_state
...
async def set_model_assignment(body: ModelAssignment):
    ...
    try:
        ...
        if scope == "main":
            ...
            # post-model-selection behaviour (robin/main.py
            ...
            if provider.strip().lower() == "nous":
                try:
                    from robin.nous_subscription import apply_nous_managed_defaults
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "webhook": {
        ...
        "docs_url": "https://robin.energyir.com/docs/user-guide/messaging/webhooks/",
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.robin.energyir.com"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Robin").strip() or "Robin"
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "docs_url": "https://robin.energyir.com/docs/guides/xai-grok-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from robin import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    if provider_id in {"anthropic", "claude-code"}:
        ...
        try:
            from robin.auth import clear_provider_auth
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from robin.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from robin.auth import _save_codex_tokens
...
async def get_logs(
    ...
    from robin.logs import _read_tail, LOG_FILES
...
#
# Wraps the same config data layer the CLI uses (robin.mcp_config), so
...
async def list_mcp_servers():
    from robin.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate):
    from robin.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str):
    from robin.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str):
    ...
    from robin.mcp_config import _get_mcp_servers, _probe_single_server
...
#
# Wraps the same JSON store the CLI uses (robin.webhook); the webhook
...
async def list_credential_pool():
    ...
    from robin.auth import read_credential_pool
...
async def list_hooks():
    ...
    try:
        from robin.plugins import VALID_HOOKS
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    _resolve_profile_dir(name)
    try:
        from robin import profile_describer
...
async def get_skills():
    ...
    from robin.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from robin.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from robin.tools_config import (
...
try:
    from robin.pty_bridge import PtyBridge, PtyUnavailableError
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from robin.dashboard_auth.audit import AuditEvent, audit_log
        from robin.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    matching what ``robin.main._launch_tui`` does for the CLI path.
    ...
    """
    from robin.main import PROJECT_ROOT, _make_tui_argv
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from robin.dashboard_auth.ws_tickets import internal_ws_credential
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`robin.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from robin.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    # (the MC Pages app), not the Robin backend. Intercept CSS asset
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as robin.plugins):
    ...
    from robin.plugins import get_bundled_plugins_dir
    ...
    # ``robin/plugins.py`` and the documented user contract.
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from robin.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from robin.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from robin.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from robin.plugins_cmd import dashboard_remove_user_plugin
...
from robin.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        from robin.dashboard_auth import list_providers
        if not list_providers():
            # Surface the *specific* reason any bundled provider declined
            # to register (e.g. missing HERMES_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Robin exposes a
