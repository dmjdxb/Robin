...
## 5. Robin Configuration

### Config location

`$HERMES_HOME/config.yaml` (defaults to `~/.robin/config.yaml` when `HERMES_HOME` is unset)
...
Restart the Robin session for changes to take effect. The MCP connection is
