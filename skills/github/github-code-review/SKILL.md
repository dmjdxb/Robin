...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.robin/.env ] && grep -q "^GITHUB_TOKEN=" ~/.robin/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.robin/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
```bash
source "${HERMES_HOME:-$HOME/.robin}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Robin. Code looks clean — good test coverage, no security concerns."
