...
Note: Tests that import ``robin.auth`` or ``robin.runtime_provider``
...
class TestResolveProvider:
    ...
    def test_auto_detect_with_aws_credentials(self, monkeypatch):
        ...
        with patch("robin.auth._load_auth_store", return_value={}):
...
class TestRuntimeProvider:
    ...
    def test_bedrock_runtime_resolution(self, monkeypatch):
        ...
        with patch("robin.runtime_provider.resolve_provider", return_value="bedrock"), \
             patch("robin.runtime_provider._get_model_config", return_value={"provider": "bedrock"}):
    ...
    def test_bedrock_runtime_no_credentials_raises_on_auto_detect(self, monkeypatch):
        ...
        with patch("robin.runtime_provider.resolve_provider", return_value="bedrock"), \
             patch("robin.runtime_provider._get_model_config", return_value={"provider": "bedrock"}), \
             patch("robin.runtime_provider.resolve_requested_provider", return_value="auto"), \
...
class TestProvidersModule:
    """Verify bedrock is wired into robin/providers.py."""

    def test_bedrock_alias_in_providers(self):
        from robin.providers import ALIASES
    ...
    def test_bedrock_transport_mapping(self):
        from robin.providers import TRANSPORT_TO_API_MODE
