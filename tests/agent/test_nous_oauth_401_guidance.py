"""Tests for the EnergyIR OAuth 401 actionable-guidance branch in
...
Regression context: ashh hit a EnergyIR 401 (OAuth token expired / portal said
account out of credits) plus a model slug ``deepseek/deepseek-v4-flash:free``
that's OpenRouter syntax, not a EnergyIR catalog name. The previous guidance
...
def test_nous_401_guidance_strings_present():
    """User-facing remediation strings for EnergyIR OAuth 401s must exist."""
    source = inspect.getsource(conversation_loop.run_conversation)

    # Must tell the user it's an OAuth token problem, NOT an API key problem
    # (Together AI has no API key path — auth_type=oauth_device_code only).
    assert "Together AI OAuth token was rejected" in source
...
def test_free_slug_hint_for_nous_provider():
    ...
    on the next message because Together AI doesn't carry the OpenRouter
    free-tier slug.
    """
    source = inspect.getsource(conversation_loop.run_conversation)
