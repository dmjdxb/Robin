...
def test_secret_capture_callback_can_be_completed_from_cli_state_machine():
    ...
    with patch("robin.callbacks.save_env_value_secure") as save_secret:
...
def test_secret_capture_uses_masked_prompt_without_tui():
    ...
    with patch("robin.callbacks.masked_secret_prompt", return_value="secret-value"), patch(
        "robin.callbacks.save_env_value_secure"
    ) as save_secret:
        save_secret.return_value = {
            "success": True,
            "stored_as": "TENOR_API_KEY",
...
def test_secret_capture_timeout_clears_hidden_input_buffer():
    ...
    with patch("robin.callbacks.queue.Queue.get", side_effect=queue.Empty), patch(
        "robin.callbacks._time.monotonic",
        side_effect=[0, 121],
    ):
        result = prompt_for_secret(cli, "TENOR_API_KEY", "Tenor API key")
