...
def adapter():
    ...
    adapter._client = SimpleNamespace(
        ...
        user=SimpleNamespace(id=99999, name="RobinBot"),
    )
...
async def test_auto_registers_plugin_commands_for_discord(adapter):
    ...
    with patch(
        "robin.plugins.get_plugin_commands",
        return_value={
            "metricas": {
...
async def test_handle_thread_create_slash_dispatches_session_when_message_provided(adapter):
    ...
    await adapter._handle_thread_create_slash(interaction, "Planning", "Hello Robin", 1440)

    adapter._dispatch_thread_session.assert_awaited_once_with(
        interaction, "555", "Planning", "Hello Robin",
    )
...
async def test_auto_create_thread_falls_back_to_hermes_when_only_mentions(adapter):
    ...
    empty — fall back to the 'Robin' default rather than ''."""
    thread = SimpleNamespace(id=999, name="Robin")
    message = SimpleNamespace(
...
async def test_auto_create_thread_falls_back_to_seed_message(adapter):
    ...
    result = await adapter._auto_create_thread(message)
    assert result is thread
    message.channel.send.assert_awaited_once_with("🧵 Thread created by Robin: **Hello**")
    seed_message.create_thread.assert_awaited_once_with(
        name="Hello",
        auto_archive_duration=1440,
...
def test_register_skill_command_is_flat_not_nested(adapter):
    ...
    with patch(
        "robin.commands.discord_skill_commands_by_category",
