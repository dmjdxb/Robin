...
class TestExtractMessagePayload:
    ...
    def test_relay_flat_format_synthesizes_chat_api_shape(self):
        ...
        GCP credentials off the Robin host. The relay flattens Chat
...
class TestTypingLifecycle:
    ...
    async def test_send_typing_concurrent_calls_create_only_one_card(self, adapter):
        ...
        'Robin is thinking…' stuck in chat. Race fix via
...
class TestGoogleChatInteractiveSetup:
    def test_interactive_setup_uses_shared_cli_prompt_helpers(self, monkeypatch):
        ...
        monkeypatch.setattr("robin.config.get_env_value", fake_get_env_value)
        monkeypatch.setattr("robin.config.save_env_value", fake_save_env_value)
        monkeypatch.setattr("robin.cli_output.prompt", fake_prompt)
        monkeypatch.setattr(
            "robin.cli_output.prompt_yes_no", lambda *_a, **_kw: True
        )
        monkeypatch.setattr(
            "robin.cli_output.print_info", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "robin.cli_output.print_success", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "robin.cli_output.print_warning", lambda *_a, **_kw: None
        )
