...
def isolated_home(tmp_path, monkeypatch):
    ...
    These tests deliberately re-import ``robin`` / ``gateway`` so each
    ...
    def fresh_gateway():
        for mod in list(sys.modules.keys()):
            if mod.startswith(("robin", "gateway")):
                del sys.modules[mod]
...
def test_lift_helper_accepts_alias_and_rejects_garbage(isolated_home):
    ...
    for mod in list(sys.modules.keys()):
        if mod.startswith("robin"):
            del sys.modules[mod]
    rp = importlib.import_module("robin.runtime_provider")
