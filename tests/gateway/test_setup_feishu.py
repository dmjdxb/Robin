"""Tests for _setup_feishu() in robin/gateway.py.
...
def _run_setup_feishu(
    *,
    ...
    with patch("robin.gateway.save_env_value", side_effect=mock_save), \
         patch("robin.gateway.get_env_value", side_effect=mock_get), \
         patch("robin.gateway.prompt_yes_no", side_effect=prompt_yes_no_responses), \
         patch("robin.gateway.prompt_choice", side_effect=prompt_choice_responses), \
         patch("robin.gateway.prompt", side_effect=prompt_responses), \
         patch("robin.gateway.print_info"), \
         patch("robin.gateway.print_success"), \
         patch("robin.gateway.print_warning"), \
         patch("robin.gateway.print_error"), \
         patch("robin.gateway.color", side_effect=lambda t, c: t), \
         patch("gateway.platforms.feishu.qr_register", return_value=qr_result):

        from robin.gateway import _setup_feishu
        _setup_feishu()
