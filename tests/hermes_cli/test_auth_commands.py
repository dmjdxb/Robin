...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from robin.auth_commands import auth_add_command
...
def test_auth_add_qwen_oauth_sets_active_provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "robin.auth.resolve_qwen_runtime_credentials",
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "robin.auth._nous_device_code_login",
        lambda **kwargs: {
    ...
    # raises "Robin is not logged into Together AI" and the agent dies.
...
def test_auth_add_minimax_oauth_starts_login_and_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "robin.auth._minimax_oauth_login",
        lambda **kwargs: {
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "robin.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "robin.auth._xai_oauth_loopback_login",
        lambda **kwargs: {
            "tokens": {
...
def test_auth_remove_reindexes_priorities(tmp_path, monkeypatch):
    ...
    from robin.auth_commands import auth_remove_command
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from robin.auth_commands import auth_reset_command
...
def test_reset_config_provider_uses_atomic_yaml_write(tmp_path, monkeypatch):
    ...
    from robin.auth import _reset_config_provider
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from robin.auth_commands import auth_list_command
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("robin.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("robin.auth_commands.time.time", lambda: 1030.0)
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from robin.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_unsuppress_credential_source_returns_false_when_absent(tmp_path, monkeypatch):
    ...
    from robin.auth import unsuppress_credential_source
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("robin.auth._import_codex_cli_tokens", _fake_import)
...
def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    ...
    from types import SimpleNamespace
    from robin.auth import is_source_suppressed
    from robin.auth_commands import auth_add_command
...
# =============================================================================
# Unified credential-source stickiness — every source Robin reads from has a
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import robin.copilot_auth as ca
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    with patch(
        "robin.copilot_auth.resolve_copilot_token",
        return_value=("ghp_fake", "gh"),
    ), patch(
        "robin.copilot_auth.get_copilot_api_token",
