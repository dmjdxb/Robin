...
from robin.codex_models import DEFAULT_CODEX_MODELS, get_codex_model_ids
...
def test_get_codex_model_ids_prioritizes_default_and_cache(tmp_path, monkeypatch):
    ...
    # backend still accepts it via the OAuth-backed CLI/Robin route.
...
def test_setup_wizard_codex_import_resolves():
    ...
    # This mirrors the exact import used in robin/setup.py line 873.
    # A prior bug had 'get_codex_models' (wrong) instead of 'get_codex_model_ids'.
    from robin.codex_models import get_codex_model_ids as setup_import
...
def test_get_codex_model_ids_adds_forward_compat_models_from_templates(monkeypatch):
    monkeypatch.setattr(
        "robin.codex_models._fetch_models_from_api",
...
def test_fetch_from_api_keeps_supported_in_api_false_models(monkeypatch):
    ...
    """
    import sys
    from robin import codex_models
...
def test_model_command_uses_runtime_access_token_for_codex_list(monkeypatch):
    from robin.main import _model_flow_openai_codex
    ...
    monkeypatch.setattr(
        "robin.auth.get_codex_auth_status",
        lambda: {"logged_in": True},
    )
    monkeypatch.setattr(
        "robin.auth.resolve_codex_runtime_credentials",
...
def test_model_command_prompts_to_reuse_or_reauthenticate_codex_session(monkeypatch, capsys):
    ...
    monkeypatch.setattr("robin.auth._login_openai_codex", _fake_login)
    monkeypatch.setattr(
        "robin.codex_models.get_codex_model_ids",
...
def test_model_command_uses_existing_codex_session_without_relogin(monkeypatch):
    ...
    monkeypatch.setattr(
        "robin.auth._login_openai_codex",
...
def _make_cli(model="anthropic/claude-opus-4.6", **kwargs):
    """Create a RobinCLI with minimal mocking."""
    ...
    with (
        ...
        cli = RobinCLI(model=model, **kwargs)
    return cli
