...
class TestReadChain:
    def test_returns_empty_list_when_unset(self):
        from robin.fallback_cmd import _read_chain
        assert _read_chain({}) == []
    ...
    def test_reads_new_list_format(self):
        ...
        cfg = {
            "fallback_providers": [
                ...
                {"provider": "nous", "model": "Robin-4-Llama-3.1-405B"},
            ]
        }
    ...
    def test_merges_new_and_legacy_formats(self):
        ...
        cfg = {
            ...
            "fallback_model": {"provider": "nous", "model": "Robin-4"},
        }
        assert _read_chain(cfg) == [
            {"provider": "openrouter", "model": "anthropic/claude-sonnet-4.6"},
            {"provider": "nous", "model": "Robin-4"},
        ]
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from robin.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestListCommand:
    def test_list_empty(self, isolated_home, capsys):
        ...
        from robin.fallback_cmd import cmd_fallback_list
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home, capsys):
        ...
        with patch("robin.main.select_provider_and_model", side_effect=fake_picker), \
                patch("robin.main._require_tty"):
            from robin.fallback_cmd import cmd_fallback_add
...
class TestRemoveCommand:
    def test_remove_empty_chain(self, isolated_home, capsys):
        _write_config(isolated_home, {})
        from robin.fallback_cmd import cmd_fallback_remove
    ...
    def test_remove_selected_entry(self, isolated_home, capsys):
        ...
        # Picker returns index 1 (the middle entry, "nous / Robin-4")
        with patch("robin.setup._curses_prompt_choice", return_value=1):
            from robin.fallback_cmd import cmd_fallback_remove
            cmd_fallback_remove(types.SimpleNamespace())
...
class TestClearCommand:
    def test_clear_empty_chain(self, isolated_home, capsys):
        _write_config(isolated_home, {})
        from robin.fallback_cmd import cmd_fallback_clear
...
class TestArgparseWiring:
    ...
    def test_fallback_help_lists_subcommands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "robin.main", "fallback", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
        )
