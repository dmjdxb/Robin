"""Tests for the Phase 4 s6 dispatch helper in robin.gateway.
...
def test_dispatch_returns_false_on_host(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from robin import gateway as gw
    monkeypatch.setattr(
        "robin.service_manager.detect_service_manager", lambda: "systemd",
    )
    # Should not even attempt to construct a manager.
    monkeypatch.setattr(
        "robin.service_manager.get_service_manager",
...
def test_dispatch_returns_true_and_calls_start_on_s6(
    ...
    monkeypatch.setattr(
        "robin.service_manager.get_service_manager", lambda: rec,
    )
...
def test_dispatch_defaults_profile_to_default(
    ...
    monkeypatch.setattr(
        "robin.gateway._profile_suffix", lambda: "",
    )
...
def test_dispatch_renders_gateway_not_registered_friendly(
    ...
    from robin import gateway as gw
    from robin.service_manager import GatewayNotRegisteredError
    ...
    monkeypatch.setattr(
        "robin.service_manager.get_service_manager", lambda: _RaisesMissing(),
    )
...
def test_dispatch_renders_s6_command_error_friendly(
    ...
    monkeypatch.setattr(
        "robin.service_manager.get_service_manager", lambda: _RaisesS6Error(),
    )
...
def _stub_s6(monkeypatch: pytest.MonkeyPatch, *, on_s6: bool) -> _CallRecorder:
    ...
    rec = _CallRecorder()
    monkeypatch.setattr(
        "robin.service_manager.detect_service_manager",
...
def test_redirect_fires_inside_s6_container(
    ...
    monkeypatch.setattr("robin.gateway._profile_suffix", lambda: "")
    ...
    monkeypatch.setattr("robin.gateway.os.execvp", fake_execvp)
    # If the fallback ran, the normal sleep path was wrongly skipped.
    monkeypatch.setattr(
        "robin.gateway._block_until_terminated",
...
def test_redirect_falls_back_when_sleep_missing(
    ...
    monkeypatch.setattr("robin.gateway.os.execvp", missing_sleep)
...
def test_block_until_terminated_installs_sigterm_handler_and_blocks(
    ...
    monkeypatch.setattr("robin.gateway.signal.pause", fake_pause)
