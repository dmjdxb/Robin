"""Tests for `_can_open_graphical_browser()` in robin.auth.
...
from robin.auth import _can_open_graphical_browser
...
def _force_platform_linux(monkeypatch):
    monkeypatch.setattr("robin.auth.sys.platform", "linux")
...
def test_non_linux_with_gui_allows(monkeypatch):
    ...
    monkeypatch.setattr("robin.auth.sys.platform", "darwin")
