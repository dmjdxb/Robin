...
argparse → cmd_chat → cli.main() → RobinCLI → AIAgent call chain.
"""
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``RobinCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting HERMES_IGNORE_RULES=1 flips RobinCLI.ignore_rules True."""
        monkeypatch.setenv("HERMES_IGNORE_RULES", "1")

        # Import RobinCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of RobinCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.RobinCLI)
        # Replicate the exact logic from cli.py RobinCLI.__init__:
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``robin/main.py`` must set
