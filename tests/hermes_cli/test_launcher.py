...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./hermes` should use `robin.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "hermes"
    called = []

    fake_main_module = types.ModuleType("robin.main")

    def fake_main():
        called.append("robin.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "robin.main", fake_main_module)
