"""
Tests for robin.mcp_config — ``hermes mcp`` subcommands.
...
def _isolate_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "robin.config.get_hermes_home", lambda: tmp_path
    )
    config_path = tmp_path / "config.yaml"
    env_path = tmp_path / ".env"
    monkeypatch.setattr(
        "robin.config.get_config_path", lambda: config_path
    )
    monkeypatch.setattr(
        "robin.config.get_env_path", lambda: env_path
    )
    return tmp_path
...
class TestMcpList:
    def test_list_empty_config(self, tmp_path, capsys):
        from robin.mcp_config import cmd_mcp_list
...
class TestMcpRemove:
    def test_remove_existing_server(self, tmp_path, capsys, monkeypatch):
        ...
        from robin.mcp_config import cmd_mcp_remove
    ...
    def test_remove_cleans_oauth_tokens(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "robin.mcp_config.get_hermes_home", lambda: tmp_path
        )
...
class TestMcpAdd:
    def test_add_no_transport(self, capsys):
        ...
        from robin.mcp_config import cmd_mcp_add
    ...
    def test_add_http_server_all_tools(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "robin.mcp_config._probe_single_server", mock_probe
        )
    ...
    def test_add_connection_failure_save_disabled(
        ...
        monkeypatch.setattr(
            "robin.mcp_config._probe_single_server", mock_probe_fail
        )
...
class TestMcpTest:
    def test_test_not_found(self, tmp_path, capsys):
        _seed_config(tmp_path, {})
        from robin.mcp_config import cmd_mcp_test
...
class TestProbeEnvResolution:
    ...
    def test_resolve_interpolates_header(self, monkeypatch):
        from robin.mcp_config import _resolve_mcp_server_config
...
class TestStripBearerPrefix:
    ...
    def test_bare_token_unchanged(self):
        from robin.mcp_config import _strip_bearer_prefix
...
class TestConfigHelpers:
    def test_save_and_load_mcp_server(self, tmp_path):
        from robin.mcp_config import _save_mcp_server, _get_mcp_servers
    ...
    def test_env_key_for_server(self):
        from robin.mcp_config import _env_key_for_server
...
class TestMcpLogin:
    def test_login_rejects_unknown_server(self, tmp_path, capsys):
        _seed_config(tmp_path, {})
        from robin.mcp_config import cmd_mcp_login
    ...
    def test_login_false_success_no_token(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "robin.mcp_config._probe_single_server",
