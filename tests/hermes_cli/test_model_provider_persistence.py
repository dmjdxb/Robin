...
class TestSaveModelChoiceAlwaysDict:
    def test_string_model_becomes_dict(self, config_home):
        ...
        from robin.auth import _save_model_choice
...
class TestProviderPersistsAfterModelSave:
    def test_update_config_for_provider_uses_atomic_yaml_write(self, config_home):
        ...
        from robin.auth import _update_config_for_provider
        ...
        with patch("robin.auth.atomic_yaml_write", side_effect=_boom) as mock_write:
    ...
    def test_api_key_provider_saved_when_model_was_string(self, config_home, monkeypatch):
        ...
        from robin.main import _model_flow_api_key_provider
        ...
        with patch("robin.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("robin.auth.deactivate_provider"), \
    ...
    def test_copilot_provider_saved_when_selected(self, config_home):
        ...
        from robin.main import _model_flow_copilot
        from robin.config import load_config

        with patch(
            "robin.auth.resolve_api_key_provider_credentials",
            return_value={
                "provider": "copilot",
            ...
            "robin.auth._prompt_model_selection",
            return_value="gpt-5.4",
        ), patch(
            "robin.main._prompt_reasoning_effort_selection",
            return_value="high",
        ), patch(
            "robin.auth.deactivate_provider",
        ):
    ...
    def test_named_custom_provider_preserves_explicit_api_mode(self, config_home):
        ...
        from robin.main import _model_flow_named_custom
        ...
        with patch("robin.auth._save_model_choice"), \
             patch("robin.auth.deactivate_provider"), \
             patch("robin.models.fetch_api_models", return_value=["gpt-5.4"]), \
             patch("robin.curses_ui.curses_radiolist", side_effect=OSError("no tty in test")), \
    ...
    def test_copilot_acp_provider_saved_when_selected(self, config_home):
        ...
        from robin.main import _model_flow_copilot_acp
        from robin.config import load_config

        with patch(
            "robin.auth.get_external_process_provider_status",
            return_value={
            ...
            "robin.auth.resolve_external_process_provider_credentials",
            return_value={
    ...
    def test_opencode_go_models_are_selectable_and_persist_normalized(self, config_home, monkeypatch):
        ...
        with patch("robin.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.7"]), \
             patch("robin.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("robin.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
    ...
    def test_opencode_go_same_provider_switch_recomputes_api_mode(self, config_home, monkeypatch):
        ...
        with patch("robin.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.5"]), \
             patch("robin.auth._prompt_model_selection", return_value="minimax-m2.5"), \
             patch("robin.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
...
class TestBaseUrlValidation:
    ...
    def test_invalid_base_url_rejected(self, config_home, monkeypatch, capsys):
        ...
        from robin.main import _model_flow_api_key_provider
        from robin.config import load_config, get_env_value

        # User types a shell command instead of a URL at the base URL prompt
        with patch("robin.auth._prompt_model_selection", return_value="glm-5"), \
             patch("robin.auth.deactivate_provider"), \
