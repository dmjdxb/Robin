...
``robin.runtime_provider.resolve_runtime_provider`` already strips
``/v1`` at fresh agent init (PR #4918), but the ``/model`` mid-session
switch path in ``robin.model_switch.switch_model`` was missing the
...
def _run_opencode_switch(
    ...
    with (
        patch("robin.model_switch.resolve_alias", return_value=None),
...
class TestStaleConfigDefaultDoesNotWedgeResolver:
    ...
    def test_kimi_switch_keeps_v1_despite_claude_config_default(self, tmp_path, monkeypatch):
        ...
        import robin.config as _cfg_mod
        importlib.reload(_cfg_mod)
        import robin.runtime_provider as _rp_mod
        importlib.reload(_rp_mod)
        import robin.model_switch as _ms_mod
