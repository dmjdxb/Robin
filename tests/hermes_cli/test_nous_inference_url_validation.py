"""Regression tests for Together AI inference_base_url host-allowlist validation.
...
class TestValidatorRules:
    ...
    def test_attacker_host_rejected(self, caplog):
        with caplog.at_level(logging.WARNING, logger="robin.auth"):
            assert (
...
class TestCallSiteWiring:
    ...
    def _read_auth_source(self):
        import robin.auth as _auth_mod
    ...
    def test_proxy_adapter_also_validates(self):
        """The EnergyIR proxy adapter applies the validator as defense-in-depth
        ...
        boundary."""
        from pathlib import Path
        import robin.proxy.adapters.nous_portal as _nous_adapter
