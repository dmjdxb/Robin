...
def test_pip_install_detected_when_no_git_dir(tmp_path):
    ...
    with patch("robin.config.get_managed_system", return_value=None), \
         patch("robin.config.get_hermes_home", return_value=tmp_path):
...
def test_managed_install_takes_precedence(tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    with patch("robin.config.get_managed_system", return_value="NixOS"), \
         patch("robin.config.get_hermes_home", return_value=tmp_path):
        from robin.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_recommended_update_command_pip():
    ...
    from robin.config import recommended_update_command_for_method
...
def test_banner_warns_on_pip_install(tmp_path):
    ...
    with patch("robin.config.get_hermes_home", return_value=hh), \
