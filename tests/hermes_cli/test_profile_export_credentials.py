...
from robin.profiles import export_profile, _DEFAULT_EXPORT_EXCLUDE_ROOT
...
class TestCredentialExclusion:
    ...
    def test_named_profile_export_excludes_auth(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("robin.profiles._get_profiles_root", lambda: profiles_root)
        monkeypatch.setattr("robin.profiles.get_profile_dir", lambda n: profile_dir)
        monkeypatch.setattr("robin.profiles.validate_profile_name", lambda n: None)
