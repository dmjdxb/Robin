"""Tests for the Phase 4 s6 hooks in robin.profiles.
...
tests/robin/test_profiles.py; here we only exercise the new
...
def test_register_calls_through_on_s6(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    mgr = _S6Manager()
    monkeypatch.setattr(
        "robin.service_manager.get_service_manager", lambda: mgr,
    )
...
def test_register_swallows_no_backend_runtime_error(
    ...
    monkeypatch.setattr(
        "robin.service_manager.get_service_manager", _no_backend,
    )
    # Should NOT raise
...
def test_register_silent_when_detect_throws(
    ...
    monkeypatch.setattr(
        "robin.service_manager.detect_service_manager", _broken_detect,
    )
