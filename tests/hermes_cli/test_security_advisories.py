"""Tests for robin.security_advisories.
...
import robin.security_advisories as adv
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full robin.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "robin.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "robin.config.save_config",
