...
from robin.setup import setup_agent_settings
...
def test_setup_agent_settings_uses_displayed_max_iterations_value(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("robin.setup.prompt", lambda *args, **kwargs: next(prompt_answers))
    monkeypatch.setattr("robin.setup.prompt_choice", lambda *args, **kwargs: 4)
    monkeypatch.setattr("robin.setup.save_env_value", lambda *args, **kwargs: None)
    monkeypatch.setattr("robin.setup.remove_env_value", lambda *args, **kwargs: None)
    monkeypatch.setattr("robin.setup.save_config", lambda *args, **kwargs: None)
