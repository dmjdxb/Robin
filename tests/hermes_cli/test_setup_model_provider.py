...
from robin.main, these tests mock the delegation point and verify
that the setup wizard correctly syncs config from disk after the call.
"""

from __future__ import annotations

from robin.config import load_config, save_config, save_env_value
from robin.nous_subscription import NousFeatureState, NousSubscriptionFeatures
from robin.setup import _print_setup_summary, setup_model_provider
...
def test_setup_copilot_acp_skips_same_provider_pool_step(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("robin.setup.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("robin.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("robin.auth.get_active_provider", lambda: None)
...
def test_setup_summary_local_browser_unavailable_without_chromium(
    ...
    monkeypatch.setattr("robin.nous_subscription._has_agent_browser", lambda: True)
    monkeypatch.setattr(
