...
import pytest
from robin.config import DEFAULT_CONFIG, load_config, save_config
...
class TestNonInteractiveSetup:
    ...
    def test_cmd_setup_allows_noninteractive_flag_without_tty(self):
        ...
        from robin.main import cmd_setup

        args = _make_setup_args(non_interactive=True)

        with (
            patch("robin.setup.run_setup_wizard") as mock_run_setup,
    ...
    def test_non_interactive_flag_skips_wizard(self, capsys):
        ...
        with (
            patch("robin.setup.ensure_hermes_home"),
            patch("robin.setup.load_config", return_value={}),
            patch("robin.setup.get_hermes_home", return_value="/tmp/.hermes"),
            patch("robin.auth.get_active_provider", side_effect=AssertionError("wizard continued")),
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        ...
        from robin.main import cmd_chat

        args = _make_chat_args()

        with (
            patch("robin.main._has_any_provider_configured", return_value=False),
            patch("robin.main.cmd_setup") as mock_setup,
            patch("sys.stdin") as mock_stdin,
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            mock_stdin.isatty.return_value = False
