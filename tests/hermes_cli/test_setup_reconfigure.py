...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("robin.setup.ensure_hermes_home", {}),
        ("robin.setup.is_interactive_stdin", {"return_value": True}),
        ("robin.config.is_managed", {"return_value": False}),
        ("robin.setup.load_config", {"return_value": {}}),
        ("robin.setup.save_config", {}),
        ("robin.setup.get_env_value", {"return_value": None}),
        ("robin.auth.get_active_provider", {"return_value": "openrouter"}),
        ("robin.setup._print_setup_summary", {}),
        ("robin.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("robin.setup.load_config", {"return_value": {}}),
        ("robin.setup.save_config", {}),
        ("robin.auth.get_active_provider", {"return_value": None}),
        ("robin.setup.get_env_value", {"return_value": None}),
        ("robin.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="robin.setup.prompt_choice",
                quick="robin.setup._run_quick_setup",
                model="robin.setup.setup_model_provider",
                terminal="robin.setup.setup_terminal_backend",
                agent="robin.setup.setup_agent_settings",
                gateway="robin.setup.setup_gateway",
                tools="robin.setup.setup_tools",
            )
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("robin.setup.prompt_choice", {"return_value": 0}),
                first="robin.setup._run_first_time_quick_setup",
            )
            from robin.setup import run_setup_wizard
            run_setup_wizard(args)
