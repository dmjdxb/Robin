...
class TestHandleSkillsSlashInstallFlags:
    ...
    def test_yes_flag_sets_skip_confirm(self):
        ...
        with patch("robin.skills_hub.do_install") as mock_install:
...
class TestHandleSkillsSlashUninstallFlags:
    ...
    def test_yes_flag_sets_skip_confirm(self):
        from robin.skills_hub import handle_skills_slash
        with patch("robin.skills_hub.do_uninstall") as mock_uninstall:
...
class TestDoInstallSkipConfirm:
    ...
    @patch("robin.skills_hub.input", return_value="n")
    def test_without_skip_confirm_prompts_user(self, mock_input):
        ...
        with patch("robin.skills_hub._console"), \
             patch("tools.skills_hub.ensure_hub_dirs"), \
             patch("tools.skills_hub.GitHubAuth"), \
             patch("tools.skills_hub.create_source_router") as mock_router, \
             patch("robin.skills_hub._resolve_short_name", return_value="test/skill"), \
             patch("robin.skills_hub._resolve_source_meta_and_bundle") as mock_resolve:
...
class TestDoUninstallSkipConfirm:
    ...
    def test_skip_confirm_bypasses_input(self):
        ...
        from robin.skills_hub import do_uninstall
        with patch("robin.skills_hub._console") as mock_console, \
