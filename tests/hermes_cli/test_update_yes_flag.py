...
from robin.main import cmd_update
...
class TestUpdateYesConfigMigration:
    ...
    @patch("robin.config.migrate_config")
    @patch("robin.config.check_config_version", return_value=(1, 2))
    @patch("robin.config.get_missing_config_fields", return_value=[])
    @patch("robin.config.get_missing_env_vars", return_value=["NEW_KEY"])
    ...
    def test_no_yes_flag_still_prompts_in_tty(
        ...
        # real ``sys`` module instead of replacing ``robin.main.sys`` with
        # a MagicMock. The MagicMock approach was flaky under ``pytest-xdist``
        # — a sibling test that imported ``robin.main`` first could leave
