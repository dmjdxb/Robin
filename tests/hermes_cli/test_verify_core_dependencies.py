...
def temp_pyproject(tmp_path, monkeypatch):
    """Point robin.main.PROJECT_ROOT at a tmp dir with a minimal pyproject.
...
class TestVerifyCoreDependencies:
    def test_no_action_when_all_deps_present(self, temp_pyproject, fake_venv_python):
        ...
        with patch("robin.main._resolve_install_target_python", return_value=py), \
             patch("robin.main.subprocess.run") as mock_run, \
             patch("robin.main._run_install_with_heartbeat") as mock_install:
            mock_run.return_value = MagicMock(returncode=0, stdout="", stderr="")

            from robin.main import _verify_core_dependencies_installed
    ...
    def test_triggers_reinstall_when_dep_missing(self, temp_pyproject, fake_venv_python):
        ...
        with patch("robin.main._resolve_install_target_python", return_value=py), \
             patch("robin.main.subprocess.run", side_effect=fake_subprocess_run), \
             patch("robin.main._run_install_with_heartbeat") as mock_install:
    ...
    def test_skips_deps_excluded_by_environment_markers(self, temp_pyproject, fake_venv_python):
        ...
        with patch("robin.main._resolve_install_target_python", return_value=py), \
             patch("robin.main.subprocess.run", side_effect=fake_subprocess_run), \
             patch("robin.main._run_install_with_heartbeat"), \
    ...
    def test_no_pyproject_is_noop(self, tmp_path, monkeypatch):
        ...
        with patch("robin.main._resolve_install_target_python") as mock_resolve, \
             patch("robin.main._run_install_with_heartbeat") as mock_install:
            from robin.main import _verify_core_dependencies_installed
...
class TestResolveInstallTargetPython:
    def test_uses_virtual_env_from_environment(self, tmp_path):
        ...
        with patch("robin.main._is_windows", return_value=True):
            from robin.main import _resolve_install_target_python
