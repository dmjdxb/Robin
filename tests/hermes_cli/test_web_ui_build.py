...
Critical invariant: the dashboard Vite build outputs to robin/web_dist/
(vite.config.ts: outDir: "../../robin/web_dist"), NOT web/dist/.
...
from robin.main import _web_ui_build_needed, _build_web_ui, _run_npm_install_deterministic
...
def _make_web_dir(tmp_path: Path) -> tuple[Path, Path]:
    ...
    dist_dir = tmp_path / "robin" / "web_dist"
...
class TestWebUIBuildNeeded:
    ...
    def test_web_dist_dir_not_web_dist_subdir(self, tmp_path):
        """Regression: sentinel must be in robin/web_dist/, NOT web/dist/."""
        web_dir, dist_dir = _make_web_dir(tmp_path)
        _touch(web_dir / "src" / "App.tsx", offset=-10)
...
class TestBuildWebUISkipsWhenFresh:
    ...
    def test_runs_npm_when_dist_missing(self, tmp_path):
        ...
        with patch("robin.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("robin.main.subprocess.run", return_value=mock_cp) as mock_run, \
             patch("robin.main._run_with_idle_timeout", return_value=build_ok) as mock_idle:
            result = _build_web_ui(web_dir)
    ...
    def test_npm_install_uses_utf8_replace_output_decoding(self, tmp_path):
        ...
        with patch("robin.main.subprocess.run", return_value=mock_cp) as mock_run:
    ...
    def test_web_build_uses_idle_timeout_helper(self, tmp_path):
        ...
        with patch("robin.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("robin.main.subprocess.run", return_value=install_cp), \
             patch("robin.main._run_with_idle_timeout", return_value=build_cp) as mock_idle:
            result = _build_web_ui(web_dir)
    ...
    def test_termux_web_install_is_workspace_scoped(self, tmp_path, monkeypatch):
        ...
        build_cp = __import__("subprocess").CompletedProcess([], 0, stdout="", stderr="")
        with patch("robin.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("robin.main.subprocess.run", return_value=install_cp) as mock_run, \
             patch("robin.main._run_with_idle_timeout", return_value=build_cp):
            result = _build_web_ui(web_dir)
...
class TestBuildWebUIRetryAndStaleFallback:
    ...
    def test_retries_build_once_on_failure(self, tmp_path):
        ...
        with patch("robin.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("robin.main._time.sleep") as mock_sleep, \
             patch("robin.main.subprocess.run", return_value=install_ok), \
             patch("robin.main._run_with_idle_timeout",
