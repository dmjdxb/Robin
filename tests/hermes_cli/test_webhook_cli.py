"""Tests for robin/webhook.py — webhook subscription CLI."""
...
def _isolate(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "robin.webhook._is_webhook_enabled", lambda: True
    )
...
class TestWebhookEnabledGate:
    def test_blocks_when_disabled(self, capsys, monkeypatch):
        monkeypatch.setattr("robin.webhook._is_webhook_enabled", lambda: False)
    ...
    def test_real_check_disabled(self, monkeypatch):
        monkeypatch.setattr(
            "robin.webhook._get_webhook_config",
            lambda: {},
        )
        monkeypatch.setattr(
            "robin.webhook._is_webhook_enabled",
            lambda: bool({}.get("enabled")),
        )
        import robin.webhook as wh_mod
