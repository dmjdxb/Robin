...
class TestRuntimeGate:
    ...
    def test_get_langfuse_does_not_import_hermes_config(self, monkeypatch):
        ...
        # Drop any cached import of robin.config.
        sys.modules.pop("robin.config", None)

        langfuse_plugin = self._fresh_plugin()
        for _ in range(20):
            langfuse_plugin._get_langfuse()

        assert "robin.config" not in sys.modules, (
            "langfuse plugin imported robin.config — regression toward "
...
# Robin logs.  The fix in `_get_langfuse()` validates the documented
