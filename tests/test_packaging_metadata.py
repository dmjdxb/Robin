...
def test_every_on_disk_subpackage_is_covered_by_packages_find():
    ...
    top-level package is listed twice — bare (``robin``) for the package
    itself and ``robin.*`` for its subpackages — EXCEPT when someone
    forgets the wildcard. v0.15.x listed ``robin`` without ``robin.*``,
    so the wheel shipped ``robin/*.py`` but dropped the ``dashboard_auth``
    and ``proxy`` subpackages. The dashboard then died on every install with
    ``ModuleNotFoundError: No module named 'robin.dashboard_auth'``.
...
def test_bundled_plugin_manifests_ship_in_both_wheel_and_sdist():
    ...
    Plugin discovery (robin/plugins.py) registers each bundled plugin by
