...
def _restore_tool_and_agent_modules():
    ...
    original_modules = {
        ...
        if name in {"tools", "agent", "robin"}
        or name.startswith("tools.")
        or name.startswith("agent.")
        or name.startswith("robin.")
    }
    try:
        yield
    finally:
        _reset_modules(("tools", "agent", "robin"))
...
def _install_fake_tools_package(*, credential_mounts=None):
    ...
    robin = types.ModuleType("robin")
    robin.__path__ = []  # type: ignore[attr-defined]
    sys.modules["robin"] = robin
    sys.modules["robin.config"] = types.SimpleNamespace(
