...
at startup, by THREE separate code paths:
  ...
  2. gateway/run.py    -> ``_terminal_env_map`` dict (gateway / messaging
                          platforms)
  3. robin/config.py:save_config_value
...
mirrors the pattern used in tests/robin/test_config_drift.py.
"""
...
def _save_config_env_sync_keys() -> set[str]:
    ...
    from robin import config as hc_config
...
def test_save_config_set_supports_critical_bridged_keys():
    ...
    assert not missing, (
        ...
        f"in robin/config.py:set_config_value."
    )
...
def test_docker_persist_across_processes_is_bridged_everywhere():
    ...
    container on Robin exit or just stops it (keeping it for the next
...
def test_docker_orphan_reaper_is_bridged_everywhere():
    ...
    ``terminal.docker_orphan_reaper`` controls whether Robin sweeps stale
