...
Lets the LLM write a Python script that calls Robin tools via RPC,
...
# available on every platform Robin itself runs on.
...
# are non-secret runtime-location flags (the same set robin treats as the
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Robin-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Robin-profile soft guard."""',
...
"""Auto-generated Robin tools RPC stubs."""
...
"""Auto-generated Robin tools RPC stubs (file-based transport)."""
...
def execute_code(
    ...
    try:
        ...
        # HERMES_TIMEZONE is an internal Robin setting and must not leak
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    description = (
        "Run a Python script that can call Robin tools programmatically. "
