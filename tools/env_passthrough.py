...
def _is_hermes_provider_credential(name: str) -> bool:
    """True if ``name`` is a Robin-managed provider credential (API key,
    ...
    Non-Robin API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Robin-managed provider credentials (from
    ...
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Robin-managed
    ...
    Non-Robin third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Robin-managed provider credentials must not be passed
