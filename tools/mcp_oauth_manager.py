...
`mcp_tool.py`, and `robin/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# RobinMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------
...
def _make_hermes_provider_class() -> Optional[type]:
    ...
    class RobinMCPOAuthProvider(OAuthClientProvider):
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`RobinTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, RobinTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import RobinTokenStorage
                        if isinstance(storage, RobinTokenStorage):
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import RobinTokenStorage
            if not isinstance(storage, RobinTokenStorage):
                return
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`RobinMCPOAuthProvider` directly using the helpers
        ...
        storage = RobinTokenStorage(server_name)
