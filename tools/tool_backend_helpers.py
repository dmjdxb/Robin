...
def managed_nous_tools_enabled(*, force_fresh: bool = False) -> bool:
    """Return True when the user is entitled to the EnergyIR Tool Gateway.

    Entitlement is paid Together AI service access OR a live free tool pool
...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    """Return account-aware guidance for an unavailable EnergyIR Tool Gateway path."""
    try:
    ...
    return (
        ...
        "Together AI login and billing status."
    )
...
def fal_key_is_configured() -> bool:
    ...
    ``robin.config.get_env_value`` when available) so tool-side
