...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.hermes/config.yaml``. Robin
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``robin.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from robin.config import get_env_value as _get_env_value
    except ImportError:
...
def _load_tts_config() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug("robin.config not available, using default TTS config")
        return {}
    except Exception as e:
...
# custom voice-cloning scripts, etc.) into Robin without any Python code
...
#           output_format: wav
#
# Robin writes the input text to a temp UTF-8 file, runs the command with
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # output_format when Robin actually needs a non-default format/override.
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Robin caches Piper voice models.
