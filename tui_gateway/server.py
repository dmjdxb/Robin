...
class _SlashWorker:
    """Persistent RobinCLI subprocess for slash commands."""
...
def resolve_skin() -> dict:
    try:
        from robin.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    try:
        from robin.models import detect_static_provider_for_model
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    if explicit and validate_toolset is not None:
        ...
        try:
            ...
            from robin.tools_config import _parse_enabled_flag
...
def _apply_model_switch(sid: str, session: dict, raw_input: str) -> dict:
    from robin.model_switch import parse_model_flags, switch_model
    ...
    custom_provs = None
    try:
        from robin.config import get_compatible_custom_providers, load_config
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        ...
        pass
    try:
        from robin.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from robin.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from robin.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from robin.config import load_config as _load_full_cfg
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Robin profile home
        ...
        try:
            ...
            if images:
                try:
                    ...
                    from robin.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from robin.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: robin.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from robin.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Robin itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Robin/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Robin. If it serves Robin/Desktop UI or another unrelated app, stop that process and report failure.",
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from robin.models import list_available_providers, normalize_provider
...
def _(rid, params: dict) -> dict:
    try:
        from robin.main import _has_any_provider_configured
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Robin provider is configured.",
                },
            )
...
def _(rid, params: dict) -> dict:
    ...
    ``robin.config.reload_env``, matching classic CLI's ``/reload``
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m robin.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "robin.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from robin.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from robin.config import is_managed, save_env_value
        from robin.inventory import build_models_payload, load_picker_context
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from robin.auth import PROVIDER_REGISTRY, clear_provider_auth
        from robin.config import remove_env_value
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        if not enabled:
            ...
            try:
                from robin.voice import stop_continuous
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from robin.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from robin.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from robin.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from robin.browser_connect import try_launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from robin.skills_hub import do_install
        ...
        if action == "browse":
            from robin.skills_hub import browse_skills
        ...
        if action == "inspect":
            from robin.skills_hub import inspect_skill
