"""Persistent slash-command worker — one RobinCLI per TUI session.
...
def _run(cli: RobinCLI, command: str) -> str:
...
def main():
    ...
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = RobinCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
