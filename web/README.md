...
Browser-based dashboard for managing Robin configuration, API keys, and monitoring active sessions.
...
cd ../
python -m robin.main web --no-open
...
`hermes dashboard` on port 9119 serves the **built** bundle from `robin/web_dist/`, not the Vite dev server — changes in `web/src/` will not appear there until you run `npm run build` and restart the dashboard (or use `web --no-open` + Vite as above).
...
This outputs to `../robin/web_dist/`, which the FastAPI server serves as a static SPA. The built assets are included in the Python package via `pyproject.toml` package-data.
...
- The dashboard preserves the EnergyIR brand uppercase aesthetic, but it is **opt-in per element, not global**.
...
| Wordmark | `Typography` + size/tracking only | Sidebar/mobile “Robin” — mixed case, no Mondwest, no `text-display` |
