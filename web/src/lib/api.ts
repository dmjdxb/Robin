...
const SESSION_HEADER = "X-Robin-Session-Token";
...
export async function fetchJSON<T>(
  ...
  if (res.status === 401) {
    ...
    // (``robin update``, ``robin gateway restart``, etc.). A tab kept
 ...
 *
 * The dashboard's gated-mode WS auth (``robin.web_server._ws_auth_ok``)
 ...
 *  - loopback / ``--insecure``: attach the ``X-Robin-Session-Token`` header.
 ...
 * EnergyIR contract V1 (the access token has no email/name claims —
